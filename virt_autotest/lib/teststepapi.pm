...
sub generateXML($) {
    ...
    $writer->startTag('testsuite', "error"=>"0", "failures"=>"$fail_nums", "hostname"=>"`hostname`", "id"=>"0", "name"=>$PRODUCT_TESTED_ON, "package"=>$PACKAGE_NAME, "skipped"=>"0", "tests"=>$case_num, "time"=>"", "timestamp"=>"");
    ...
    foreach my $item ( keys (%my_hash) ) {
        ...
        $writer->characters("Status:" . $case_status . ";Time:" . $my_hash{$item}->{"time"});
